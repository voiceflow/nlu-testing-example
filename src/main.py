...
from src.interface import NLUTester


def format_entities(entities):
    """Formats the entities passed in as tests into a more readable format"""
    f = {}
    for i in entities:
        if i == '':
            continue
...
def format_vf_entities(entities):
    """Extracts the VF json entity format"""
    entities_new = {}
...
class VoiceflowNLUTester(NLUTester):
    ...
    def __init__(self, data: dict[str, list[list[str]]], dm_key, version=None, test_cast_path=None):
        ...
        if dm_key == '':
            raise ValueError("No dialogue manager key provided. Open https://creator.voiceflow.com/ and create one under the settings tab in your project")
    ...
    def index_mapper_initializer(self) -> tuple[dict[str,int], dict[str,int]]:
        ...
        # these are the corresponding classes for each entity
        # ex size, topping -> size=0, topping=1
            ...
            for combo in v:
                ...
                for k, v in entities.items():
                    if k != '' and not (k in entity_index_mapper.keys()):
                        ...
                        e_i += 1
        # creates the intent classes
        # 0 is the none intent
